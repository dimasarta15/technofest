    ...
    <div class="content-wrapper">
        <div class="row">
            <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <a href="{{ route(getLang() . 'backsite.major.create') }}" class="btn btn-primary btn-rounded btn-sm"><i class="mdi mdi-account-plus"></i> @trans('backsite.major.addata') </a>
...
<script>
    ...
    function getDatatable() {
        ...
        $('.dtable').DataTable({
            ...
            ajax: {
                type: 'GET',
                url: "{{ route(getLang() . 'backsite.major.datatable') }}",
                data: {
                    _token: "{{ csrf_token() }}",
                    type: $('#type').val()
                }
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return ` 
                            <form action="${route(getLang() + 'backsite.major.destroy', row.id)}" method="POST" id="form_${row.id}">
                                @csrf
                                ...
                                <a type="button" rel="tooltip" class="btn btn-sm btn-success btn-round" data-original-title="" title="" href="${route(getLang() + 'backsite.major.edit', {
                                    id: row.id
                                })}">
